...
- NAS
  ...
  - [ ] Бекапы внешние
    ...
    - [ ] Компьютер -> NAS
      - [ ] Скорее NAS <- Компьютер. То есть NAS вытягивает данные с компа для бекапа. Чтобы при смене компа не нужно было почти ничего настраивать, только дать доступ
        - [ ] rsync
  - [x] Синхронизации
    - [x] Google Photo
  - [ ] Сервисы
  ...
  - [ ] Виртуальные машины
    - [ ] Настройка
      - [x] Поставил docker контейнер, но пока корректно не работает. Кладет сеть
      - [ ] Настроить контейнер
